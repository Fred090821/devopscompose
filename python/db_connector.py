...
def connect_to_database():
    try:
        conn = pymysql.connect(user='user', password='password', host='0.0.0.0', database='mydb')

        if conn:
            return conn
        else:
            print(f"Mysql connection failed using user: %s, password: %s, host:%s, database: %",
                  'user', 'password', 'mysql', 'mydb')
...
def get_next_available_row_id_from_db():
    ...
    try:
        ...
        if not conn:
            print('Database connection error. Please check the code base')
        else:
            cursor = conn.cursor()
    ...
    try:
        ...
        if not conn:
            ...
            try:
                ...
                cursor.execute(select_query, ('http://0.0.0.0:5003/users/', 'john'))
