...
def get_user(user_id):
    ...
    if request.method == 'GET':
        ...
        try:
            ...
            try:
                print(f"retrieve user with user_id = %s ;", user_id)
                cursor.execute(f"SELECT * FROM {config.db_name}.users WHERE user_id = %s;", user_id)
                result = cursor.fetchone()

                if result:
                    print(f"retrieve user with user_id = %s has been retrieved;", user_id)
                    print(f"RESPONSE FROM QUERY  %s ", result[1])
                    # return jsonify(result)  # Return the result as JSON
                    return jsonify({"status": "ok", "user_name": result[1]}), 200

                else:
                    print(f"User with id %s does not exist;", user_id)
    ...
    if request.method == 'PUT':
        ...
        try:
            ...
            if not existing_user:
                print("Can't update user with id  %s that does not exist", user_id)
                ...
                conn.commit()
                print("Update user ...with id %s", user_id)
    ...
    if request.method == 'DELETE':
        print("Entered DELETE method call")
        conn = None
        cursor = None
        ...
        try:
            ...
            if not existing_user:
                ...
                conn.commit()
                print("Deleted entry from the database...with id %s", user_id)
...
def create_user():
    print("Entered POST method call")
    ...
    try:
        ...
        if existing_user:
            ...
            print(f"new id after duplicate %s", new_id_for_duplicate_name)
        else:
        ...
        conn.commit()
        print("Add new entry to the database...with id %s", new_id_for_duplicate_name)
        return jsonify(
